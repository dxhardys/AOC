...
int load_values(const char *file_name, value_grid_t *val_grid)
{
    ...
    // Allocate memory for the grid at once, instead of reallocating on every iteration
    unsigned total_values = nx * ny;
    val_grid->entries = malloc(total_values * sizeof(value_t*));  
    sum_bytes += total_values * sizeof(value_t*);

    if (val_grid->entries == NULL) {
    ...
    while (nb_inserted_values < total_values) {
        // Parse current line (v1, v2) directly with fscanf
        float v1, v2;
        if (fscanf(fp, "%f %f", &v1, &v2) != 2) {
            fprintf(stderr, "Failed to parse a line from the input file\n");
            fclose(fp);
            return 1;
        }

        // Allocate and set value_t in the entries array
    ...
    // Check if the number of inserted values matches the grid size
    if (nb_inserted_values != total_values) {
